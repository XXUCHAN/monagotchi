...
Monad Testnet에서 Chainlink Data Feeds를 활용해 **Oracle Imprint Cat**을 온체인으로 키우는 소셜 펫 게임 컨트랙트를 구현한다. 사용자는 BTC/ETH(및 확장 가능한 여러 클랜) 진영의 고양이를 민팅하면, 민팅 시점 시장 상태가 고양이의 성격/희귀도/포츈으로 새겨지고, 이후 미션·룰렛을 통해 파워를 키워 FISH 토큰 보상을 받을 수 있다.
...
2. **Contract Tests First**
   ...
   - FISH 보상 조건(`power` 임계값, rewarded 플래그 등)과 권한 에러 케이스를 명시적으로 테스트.
...
4. **Mission / Roulette & Reward Loop**
   ...
   - `claimReward`에서 파워 임계값 및 rewarded 플래그를 이용해 FISH 보상을 1회만 지급하도록 구현 (T024).

5. **Integration & Handoff**
   - `FishToken.sol`과의 통합(보상 지급, owner 권한 위임)을 테스트로 검증하고, Monad Testnet용 배포 스크립트를 최신 상태로 유지 (T020, T030~T033).
...
- [ ] Phase 0 완료: 리서치 및 환경 설정
- [ ] Phase 1 완료: 설계 산출물 작성
- [ ] Phase 2 완료: 작업 계획 수립
- [ ] 컨트랙트 구현 완료
- [ ] 테스트 작성 및 통과
- [ ] 배포 및 검증
