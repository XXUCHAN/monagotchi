...
플레이어는 Monad Testnet에서 BTC/ETH 등 여러 클랜 중 하나를 선택해 **Oracle Imprint Cat**을 민팅하고, 민팅 시점의 시장 상태가 새겨진 능력치·성격을 기반으로 미션/룰렛을 수행하여 파워를 키우고 FISH 토큰 보상을 받는 온체인 다마고치 게임을 즐길 수 있다.
...
3. Given 선택한 클랜의 변동성 버킷이 “High” 이상일 때, When 특수 변동성 룰렛 미션을 실행하면, Then 일반 미션 대비 더 큰 파워 증가 또는 추가 FISH 보상 중 하나가 확률적으로 지급된다. `[NEEDS CLARIFICATION: High 버킷 기준과 보너스 분포]`
4. Given 고양이 파워가 보상 임계값 이상이고 rewarded=false 일 때, When 보상 수령을 요청하면, Then FISH 토큰이 1회에 한해 지급되고 rewarded=true로 변경된다.
...
- **FR-007**: 고양이 파워가 설정된 임계값(예: 50) 이상일 때 FISH 토큰 보상을 1회에 한해 수령할 수 있어야 한다.
...
interface OracleImprint {
    clan: number;           // 클랜: 0=BTC, 1=ETH, 2=SOL, 3=MONAD ...
...
- **INVALID_ALIGNMENT**: alignment 값이 0 또는 1이 아닌 경우
